...
// - Custom Definitions
#define TOTALANIMALS      6     // Total animals that have stickers
#define TOTALSTICKERS     4     // Total number of stickers per animal (must be highest amount no matter what, and all arrays of strings must have at least this many)

/* Card Definitions */
/* ---
NOTE:
CHANGE THE VALUES IN THE QUOTATIONS FOR YOUR NFC CARD UID (Serial Number)
YOU CAN ADD MORE QUOTATIONS WITH COMMAS IN BETWEEN AS SHOWN BELOW FOR MORE THAN ONE CARD PER ANIMAL
--- */

const char* cowCards[] = {"043CB442A51C90", "0432B442A51C90", "0438B442A51C90", "042EB442A51C90"};
const char* dogCards[] = {"0444B442A51C90", "044CB442A51C90", "0448B442A51C90", "0460B442A51C90"};
const char* sheepCards[] = {"042AB442A51C90", "0426B442A51C90", "0422B442A51C90", "041EB442A51C90"};
const char* horseCards[] = {"undefined", "0454B442A51C90", "0458B442A51C90", "0450B442A51C90"};
const char* chickenCards[] = {"041AB442A51C90", "0412B442A51C90", "0416B442A51C90", "040EB342A51C90"};
const char* pigCards[] = {"0440B442A51C90", "041AB342A51C90", "0416B342A51C90", "041EB342A51C90"};
...
void checkAnimal()
{
  for (uint8_t i = 0; i < TOTALANIMALS; i++)
    for (uint8_t j = 0; j < TOTALSTICKERS; j++)
      switch (i)
      {
        case 0: if (!strcmp(cowCards[j], uid)) { currentFile = COW; return; } break;
        case 1: if (!strcmp(dogCards[j], uid)) { currentFile = DOG; return; } break;
        case 2: if (!strcmp(sheepCards[j], uid)) { currentFile = SHEEP; return; } break;
        case 3: if (!strcmp(horseCards[j], uid)) { currentFile = HORSE; return; } break;
        case 4: if (!strcmp(chickenCards[j], uid)) { currentFile = CHICKEN; return; } break;
        case 5: if (!strcmp(pigCards[j], uid)) { currentFile = PIG; return; } break;
        // case 6: if (!strcamp([ANIMALNAME]Cards[i], uid)) { currentFile = [ANIMALNAME]; return; } break; // !!-- ANIMAL NAME EXAMPLE
        default: return;
      }
}
...
void checkCharge()
{
  ...
  switch (charge)
  {
    case 0: dacWrite(LED_PIN, 200); break;  // Green
    case 1: dacWrite(LED_PIN, 180); break;  // Yellow
    case 2: dacWrite(LED_PIN, 160); break;  // Red
